...
程序每次状态变化都会把快照写入 `data/grid-state.json`（自动生成，含配置、挂单、待确认下单、近期成交记录和累计统计）。关键交易动作及每笔下单使用同步原子落盘；Docker / Dokploy 部署必须把 `/app/data` 挂载为持久卷。重启后：

1. **上次是运行状态** → 自动**续跑**：重新连接交易所，按市场名称重新解析交易对（交易所每次连接会重新编号市场 ID），接管还挂着的单，对账后继续运行。
2. **续跑安全校验失败**（模式不一致、市场无法确认、模拟账本不完整、停机期间部分成交/订单消失，或交易所无法提供可验证的剩余量）→ 停止自动操作并保留挂单，同时锁定新的下单/撤单接口并让健康检查返回 503；人工在交易所核对挂单和仓位、按 FAQ 显式重置后才能重新交易。当前 RISEx SDK 的挂单列表不能证明部分成交后的剩余量，因此 RISEx live 重启会保留记录和原单、锁定自动交易供人工核对，不会猜测式续跑。只有已确认同一市场后发生的运行错误才尝试撤销该市场遗留挂单。
...
**Q：怎么彻底重置程序？**
先在界面停止三个网格并撤销挂单、核对并处理遗留仓位 → 再关程序 → 把 `data/grid-state.json` 内容改为 `{}`（和 `.env` 如果想清配置）→ 重新启动。**不要在实盘网格仍运行时直接删快照**；迁移过旧版快照的部署也不要只删该文件，否则系统会因状态异常而拒绝启动。
